...
3. **مظهر Midnight Aurora والشفافية (Windows 10 & 11)**:
   - خلفية سوداء ناصعة بنسبة شفافية 32% (68% Opacity) مريحة للعين ومتزامنة تلقائياً بين الكونسول الكلاسيكي (`conhost.exe`) وترمينال ويندوز 11 الحديث (`Windows Terminal / wt.exe`).
...
| `cpf` | اختيار تفاعلي لملف واحد أو عدة ملفات عبر fzf (تحديد بـ `Tab`، معاينة بـ `Ctrl+P`، خيارات `-abs`, `-md`, `-s`, `-c`) ونسخ المسارات للحافظة |
