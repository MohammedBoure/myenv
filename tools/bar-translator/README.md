...
- **Consolidated Settings & Dropdown Menu**: Spawns a sleek dark-themed context menu containing all translation controls (Auto-Capture, Focus Mode, Bilingual display, Copy, Clear) and dynamic container/widget visibility toggles for the status bar.
...
- **Local HTTP API (`http://127.0.0.1:49876`)**: Exposes `/state`, `/copy`, `/clear`, `/translate`, `/toggle_auto_capture`, `/toggle_translation_mode`, `/toggle_show_english`, `/get_widgets`, `/toggle_widget`, and `/set_widget` endpoints.
...
| [`StateManager.cs`](file:///C:/Users/moham/Documents/myenv/tools/bar-translator/StateManager.cs) | Manages atomic state & settings persistence to `state.json` and runs local HTTP server on port 49876 with REST API for Zebar. |
