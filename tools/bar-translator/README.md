...
- **Consolidated Settings & Dropdown Menu**: Spawns a dark-themed context menu containing all translation controls (Auto-Translate Copied Text, Auto-Select Capture, Focus Mode, Bilingual display, Copy, Clear) and dynamic container/widget visibility toggles for the status bar.
...
- **Speech Synthesis & Pronunciation API**: Integrated English text-to-speech engine using `System.Speech.Synthesis.SpeechSynthesizer` capable of pronouncing captured English sentences via CLI (`--speak`) or REST endpoint (`/speak`).
...
- **Local HTTP API (`http://127.0.0.1:49876`)**: Exposes `/state`, `/reload_state`, `/copy`, `/clear`, `/speak`, `/translate`, `/toggle_clipboard_translate`, `/toggle_auto_capture`, `/toggle_translation_mode`, `/toggle_show_english`, `/get_widgets`, `/toggle_widget`, and `/set_widget` endpoints.

## Files & Structure

| File | Purpose |
|---|---|
| [`BarTranslator.csproj`](file:///C:/Users/moham/Documents/myenv/tools/bar-translator/BarTranslator.csproj) | .NET 10 project definition targeting `net10.0-windows` (WinExe mode with Windows Forms and System.Speech for zero window flash). |
| [`Program.cs`](file:///C:/Users/moham/Documents/myenv/tools/bar-translator/Program.cs) | Application entry point, single-instance mutex (`MyEnv_BarTranslator_Daemon`), CLI dispatcher including `--speak`, dark context menu renderer, and Win32 message loop. |
...
| [`StateManager.cs`](file:///C:/Users/moham/Documents/myenv/tools/bar-translator/StateManager.cs) | Manages atomic state & settings persistence to `state.json`, FileSystemWatcher sync, local HTTP server on port 49876 with REST API, and English SpeechSynthesizer execution. |
