...
Write-Host "`n[1/4] Applying Taskbar Auto-Hide..." -ForegroundColor Cyan
& "$myenvPath\scripts\set-taskbar-autohide.ps1"

# 2. Apply PowerShell Ctrl+Backspace
Write-Host "`n[2/4] Configuring Ctrl+Backspace word deletion..." -ForegroundColor Cyan
& "$myenvPath\scripts\set-ctrl-backspace.ps1"

# 3. Create Junctions
Write-Host "`n[3/4] Verifying Directory Junctions..." -ForegroundColor Cyan
$junctions = @(
...
# 4. Manage Services (Stop Zebar & Reload YASB)
Write-Host "`n[4/4] Managing Bar Services..." -ForegroundColor Cyan
